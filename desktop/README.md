...
7d  ▓▓▓▓▓░░░░  68%      204/300
─────────────────────────────────
下次重置 16:50（23m 后）· 5h +2%
```

- 拖拽球或卡片头部换位置；卡片右上角是置顶开关
- 取不到数据时，倒计时那行变成红色的「离线」——上面的百分比此刻是旧值，必须看得出来
...
最早的那次重置常常毫无价值。上面那个例子里，23 分钟后重置的是一个已经剩 98% 的 5h
窗口，等它只补 2%；真正值得等的是 18:10 那个 48% 的账号（+52%）。只报时间会让人白
等，所以那一行必须带上补多少。

### 展开方向

球多半被摆在屏幕右下角，一律向右下展开会让卡片整片跑出屏幕。`expand_float` 会读
`work_area`（排除菜单栏和 Dock）判断朝哪边翻，收回时再把球放回原来那个角。
